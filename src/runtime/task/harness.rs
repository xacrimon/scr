...
impl RawTask {
    ...
    /// the shutdown, so that the future is always dropped from the run queue
    /// rather than from whatever call site invoked `abort`.
    ...
    S: Schedule,
{
    ...
    /// Polls the task and cancels it if necessary. This takes ownership of a
    /// ref-count.
    ///
    ...
    pub(super) fn shutdown(self) {
        if !self.state().transition_to_shutdown() {
            // The task is currently running further up the stack. No further
            // work needed.
            self.drop_reference();
            return;
        }
    ...
    pub(super) fn dealloc(self) {
        ...
        //
        // Dropping the `Box` drops the future or output, the join waker, and
        // the scheduler handle held by the task.
        unsafe {
    ...
    pub(super) fn drop_join_handle_slow(self) {
        // Unset `JOIN_INTEREST`. This must be done as a first step in case the
        // task completed while the handle was being dropped.
        let drop_output = self.state().transition_to_join_handle_dropped();

        if drop_output {
            // It is our responsibility to drop the output. This is critical as
            // the task output may not be `Send` and as such must remain with
            // the scheduler or `JoinHandle`.
            //
        ...
        // the waker field, and the `JoinHandle` being dropped here is its only
        // other accessor.
    ...
    fn complete(self) {
        ...
        let _ = panic::catch_unwind(panic::AssertUnwindSafe(|| {
            if snapshot.is_join_interested() {
                // Notify the `JoinHandle`, if it has stored a waker. The
                // `JoinHandle` is responsible for taking the output.
                self.trailer().wake_join();
            } else {
                // The `JoinHandle` is not interested in the output of this
                // task. It is our responsibility to drop the output. The join
                // waker was already dropped by the `JoinHandle`.
                self.core().drop_future_or_output();
            }
        }));
...
/// Returns `true` if the task output can be read, storing `waker` as the join
/// waker otherwise.
///
/// Unlike in `tokio`, the `JoinHandle` and the runtime cannot touch the waker
/// field at the same time, so the `JOIN_WAKER` access-control bit and the
/// protocol around it are gone: the `JoinHandle` simply owns the field until it
/// unsets `JOIN_INTEREST`.
fn can_read_output(header: &Header, trailer: &Trailer, waker: &Waker) -> bool {
    ...
    // The task is not complete; store the provided waker so that the runtime
    // notifies it on completion.
    //
    // Safety: `JOIN_INTEREST` is set, so this call is coming from the
    // `JoinHandle`, which owns the field.
    unsafe {
        if !trailer.will_wake(waker) {
            trailer.set_waker(Some(waker.clone()));
        }
    }
