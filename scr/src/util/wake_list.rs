...
pub(crate) struct WakeList {
    inner: [MaybeUninit<LocalWaker>; NUM_WAKERS],
    curr: usize,
}

impl WakeList {
    pub(crate) fn new() -> Self {
        const UNINIT_WAKER: MaybeUninit<LocalWaker> = MaybeUninit::uninit();
    ...
    pub(crate) fn push(&mut self, val: LocalWaker) {
    ...
    pub(crate) fn wake_all(&mut self) {
        ...
        let mut guard = {
            let start = self.inner.as_mut_ptr().cast::<LocalWaker>();
...
impl Drop for WakeList {
    fn drop(&mut self) {
        let slice =
            ptr::slice_from_raw_parts_mut(self.inner.as_mut_ptr().cast::<LocalWaker>(), self.curr);
